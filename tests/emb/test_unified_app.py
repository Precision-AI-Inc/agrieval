...
"""Unit tests for the unified FastAPI application factory and CLI entry point.

Mirrors tests/emb/test_app.py's TestApp/TestMain pattern, but for
precisionai.agrieval.api.app — the server that mounts emb, seg, and dpt
together.
"""
...
from precisionai.agrieval.api.app import app, main
...
class TestApp:
    ...
    def test_emb_router_mounted(self) -> None:
        paths = set(app.openapi().get("paths", {}).keys())
        assert any("/v1/embeddings/evaluate" in p for p in paths)

    def test_seg_router_mounted(self) -> None:
        paths = set(app.openapi().get("paths", {}).keys())
...
class TestMain:
    ...
    def _run_main(self, argv: list[str]) -> MagicMock:
        mock = MagicMock()
        with patch("uvicorn.run", mock), patch("sys.argv", ["precisionai-agrieval-api", *argv]):
            main()
        return mock
    ...
    def test_app_string_passed_to_uvicorn(self) -> None:
        ...
        assert args[0] == "precisionai.agrieval.api.app:app"

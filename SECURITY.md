...
- Unauthorized access to embeddings, metadata, or evaluation results served through the API
- Arbitrary code execution triggered by a crafted embedding payload, metadata document, or image path
- Privilege escalation through the FastAPI service
- Exposure of sensitive agronomic or field-imagery data
- Supply-chain compromise of the package or its release artifacts
- Denial of service caused by malformed or adversarially large input (embeddings, metadata, or `k_values`) where the service is expected to handle untrusted data
